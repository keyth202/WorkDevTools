...
type FileVoice = "en-US-Neural2-A"|"en-US-Neural2-C"| "en-US-Neural2-D"|"en-US-Neural2-E"|"en-US-Neural2-F"|"en-US-Neural2-G"|"en-US-Neural2-H"|"en-US-Neural2-I"|"en-US-Neural2-J"|
...
const defaultInitial: TtsJob = {
  ...
  languageCode: "en-US",
  ssmlGender: "FEMALE",
  fileVoice: "en-US-Neural2-F",
  items: [
    { filename: "greeting.mp3", ssml: "<speak>Hello <break time=\"200ms\"/> world!</speak>" },
  ],
};

const TTSForm = ({ onSubmit, endpoint, initial, persistKey = "tts_job_form" }: Props) => {
 // --- State ---
   ...
   const [fileVoice, setFileVoice] = useState<string>("");
   ...
   useEffect(() => {
     ...
     if (raw) {
       try {
         ...
         setFileVoice(saved.fileVoice ?? fileVoice);
   ...
   const jsonPreview = useMemo(() => JSON.stringify(toPayload(), null, 2), [outputFolder, audioType, languageCode, ssmlGender, fileVoice, items]);
   ...
   const errors = useMemo(() => {
     ...
     if (!fileVoice.trim()) errs.push("File voice is required");
   ...
   return (
     <div className="min-h-screen bg-slate-950 text-slate-100 p-6">
       <div className="max-w-6xl mx-auto bg-slate-900/60 backdrop-blur border border-slate-700/50 rounded-2xl p-6 shadow-xl">
         <h1 className="text-2xl font-bold">SSML to Speech Batch Builder</h1>
         ...
         <form onSubmit={handleSubmit} className="mt-6 space-y-6">
           ...
           <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
             ...
             <div>
               <label className="text-sm text-slate-400">File voice</label>
               <select
                 value={fileVoice}
                 onChange={(e) => setFileVoice(e.target.value as FileVoice)}
                 className="mt-1 w-full rounded-xl bg-slate-950 border border-slate-700 px-3 py-2 focus:outline-none focus:ring-2 focus:ring-violet-400"
               >
                 {VOICES.map((g:any, idx:number) => (
                   g.gender == ssmlGender? <option key={idx} value={g.voice}>{g.voice}</option> : null
                 ))}
               </select>
             </div>
           </div>
