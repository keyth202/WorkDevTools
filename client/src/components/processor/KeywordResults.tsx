...
const KeywordResults = () => {
    ...
    const [postTranscripts, { data:transcriptData, isError:transcriptIsError, error:transcriptError }] = usePostKeyTranscriptsMutation();
    ...
    useEffect(() => {
        ...
        if(keywordSt.search && keywordSt.search.length > 0 && isProcessing) {       
           addLogEntry("Processing started...");
            ...
            addLogEntry(`Processing keywords ${keywordSt.keywords.join(', ')} `);
            for(const conv of keywordSt.search) {
                addLogEntry(`Processing conversations with keyword: ${conv?.conversationId}`);
                getURL(conv, conv?.conversationId, conv?.communicationId);
            }
